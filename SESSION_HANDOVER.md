...
>
> **Git:** `main` tip = **`f77f56e`** — pushed, in sync with origin (the `live_integration` branch is merged in). **Tests: 265 passed / 0 failed.** Repo: `github.com/ramachandranbalasubrahmanian-svg/Ram-Most-Trusted-Trade-App`.
> **Running:** dashboard via launchd (`com.ramistp.dashboard`, KeepAlive) on **http://127.0.0.1:8787** (`serve 30min`), on **2026-07-01 intraday data + rebuilt edge maps (30min: 1629 edges)**. (2026-07-02 is a fresh trading day — its session data lands after the 16:00 refresh; edge maps are still July-1 until then.)
...
> **⚠ Finder warm-up gotcha:** after ANY app restart the first `/api/finder|staging|picks` call rebuilds the process-wide `FIT_UNIVERSE_CACHE` (~90–120s scanning all 1767 symbols, BLOCKING that one request), then it's cached all day (~10 ms/call). Don't restart the dashboard right before demoing `/picks`. Open item: add a startup background pre-warm so the first click is instant (offered, not built).
>
